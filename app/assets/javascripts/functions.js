$(window).scroll(function(){
  ...
  if(wScroll > $('#portfolio').offset().top - $(window).height()){
     ...
     var offset2 = (Math.min(0, wScroll - $('#portfolio').offset().top +$(window).height() - 850)).toFixed();

     $('.post2').css({'transform': 'translate('+ offset2 + "." + 2 + 'px, '+ Math.abs(offset2 * 0.1) +'px)'});

      var offset3 = (Math.min(0, wScroll - $('#portfolio').offset().top +$(window).height() - 950)).toFixed();
