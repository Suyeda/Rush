$(window).scroll(function(){
  ...
  if(wScroll > $('#portfolio').offset().top - $(window).height()){
     ...
     var offset = (Math.min(0, wScroll - $('#portfolio').offset().top +$(window).height() - 750)).toFixed();

     $('.post1').css({'transform': 'translate('+ offset + "." + 1 + 'px, '+ Math.abs(offset * 0.1) +'px)'});

     var offset2 = (Math.min(0, wScroll - $('#portfolio').offset().top +$(window).height() - 1050)).toFixed();

     $('.post2').css({'transform': 'translate('+ offset2 + "." + 2 + 'px, '+ Math.abs(offset2 * 0.1) +'px)'});

      var offset3 = (Math.min(0, wScroll - $('#portfolio').offset().top +$(window).height() - 1350)).toFixed();
